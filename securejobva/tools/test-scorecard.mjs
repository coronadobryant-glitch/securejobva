/* The interview scorecard in /admin, driven against the real scoreLine().
   ...
   So this asserts what 065's scorecard is: the four things only a conversation
...
console.log("\n  The four only a call can answer");
{
...
console.log("\n  Rows written before tracks existed");
{
  ...
  is("and gives exactly one job row", count(h, "data-score="), 5);
}
{
  const none = { tracks: [] };
  const h = scoreLine(none);
  is("somebody with no track still gets the four", count(h, "data-score="), 4);
...
}
{
  ...
  is("a track this site does not offer scores nothing",
     count(scoreLine(gone), "data-score="), 4);
}
...
console.log("  Four things a call shows, then the jobs she actually applied for.");
