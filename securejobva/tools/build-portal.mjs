...
/* The typing test, back on this page.

   It was moved to another site because a number she types into a box is not
   evidence and a link to a result page is something a person can open. That
   was right about the evidence and wrong about the cost: every application
   then needed somebody to open two links and copy two numbers across, which
   is a job that does not survive the first dozen applicants.

   So the passage is typed here and the browser measures it. That is still the
   browser's word, and it goes in the columns that mean "what she reported"
   rather than the two that mean "somebody checked" — those stay yours to
   overrule after the interview. But it is a measurement of an actual typing
   run instead of a number she liked the look of, and the accuracy is measured
   against the passage rather than asserted.

   Pasting is refused, for the same reason and with the same honest limit as
   the written reply: a speed bump, not a wall.

   The connection speed test stays a link. That one genuinely cannot be
   measured from in here, and it is the thing that stops somebody starting a
   client on a line that cannot hold a call. */
function typingPart() {
  partShell("Typing and accuracy", 0,
    '<p class="msg" style="margin:1rem 0">Type the message below exactly as it is written. ' +
      "We time it from your first keystroke, so start when you are ready &mdash; and type it as you " +
      "would really send it. Accuracy counts for more than speed: work somebody has to redo is " +
      "slower than typing slowly.</p>" +

    '<p style="margin:0 0 1rem;padding:.9rem 1rem;border-radius:8px;background:var(--paper);' +
      'border:1px solid var(--line);font-size:.95rem;line-height:1.7">' + esc(TYPE_TEXT) + "</p>" +

    '<div class="note" style="margin-bottom:1.1rem"><b>Type it yourself.</b> ' +
      "Pasting is turned off here, and the number that counts is the one measured while you " +
      "type &mdash; there is nothing to fill in.</div>" +

    '<label class="a-lbl" for="a-type">Type it here</label>' +
    '<textarea id="a-type" rows="6" style="width:100%;margin:.4rem 0 .3rem" ' +
      'placeholder="Start typing the message above"></textarea>' +
    '<p class="msg" id="a-live" style="margin:.3rem 0 1.3rem">Not started yet.</p>' +
  ...
  var live = document.getElementById("a-live");
  var conn = document.getElementById("a-conn");
  var started = 0;

  /* Everything that puts text in without it being typed. Same pair as the
     written reply, and the same message taken down again the moment she
     starts typing, so a refused paste does not follow her to the end. */
  ["paste", "drop"].forEach(function (ev) {
    box.addEventListener(ev, function (e) {
      ...
      if (w) {
        w.style.display = "";
        w.textContent = "Type it yourself — pasting is turned off on this part.";
      }
    });
  });

  /* Gross words a minute, the way every typing test counts it: five characters
     is a word. Accuracy is the edit distance against the passage, which is the
     one number here that cannot be inflated by typing faster. */
  function figures() {
    var typed = box.value;
    var secs = started ? (Date.now() - started) / 1000 : 0;
    if (!typed.length || secs < 1) return { wpm: 0, acc: typed.length ? typingAccuracy(typed) : 0, secs: secs };
    var wpm = Math.round((typed.length / 5) / (secs / 60));
    return { wpm: Math.max(0, Math.min(250, wpm)), acc: typingAccuracy(typed), secs: secs };
  }

  box.addEventListener("input", function () {
    if (!started) started = Date.now();
    var f = figures();
    live.textContent = f.secs < 1
      ? "Timing from your first keystroke\u2026"
      : f.wpm + " wpm, " + f.acc + "% accurate so far";
    var w = document.getElementById("a-err");
    if (w && w.textContent.indexOf("pasting is turned off") > -1) {
      w.textContent = "";
  ...
  document.getElementById("a-done").addEventListener("click", function () {
    ...
    var f = figures();
    /* Enough of it, and long enough, for the numbers to mean anything. Four
       words in two seconds is a very fast typist by arithmetic and by nothing
       else. */
    if (box.value.length < 60 || f.secs < 5) {
      return fail("Type the message above first — we measure it as you go.", box);
    }
