...
/* a note that reports something missing, not something achieved */
.apt__s--none{color:var(--muted)}
.apt__go{padding:.45rem .9rem;font-size:.88rem}
/* the times she is offered */
.iv{list-style:none;margin:1rem 0 0;padding:0;display:grid;gap:.6rem}
.iv__s{display:flex;justify-content:space-between;align-items:center;gap:1rem;
  border:1px solid var(--line);border-radius:10px;padding:.8rem 1rem;background:var(--paper)}
.iv__s--on{background:var(--accent-soft);border-color:var(--accent)}
.iv__s b{display:block;font-weight:700}
.iv__alt{display:block;font-size:.82rem;color:var(--ink-2);margin-top:.15rem;
  font-family:'IBM Plex Mono',ui-monospace,monospace}
.iv__note{font-size:.85rem;color:var(--accent);font-weight:700;white-space:nowrap}
.iv__go{padding:.45rem .9rem;font-size:.88rem;white-space:nowrap}
/* A link, not a button: saying none of them work is the quiet option and
   should not compete with choosing one. */
.iv__none{display:inline-block;margin-top:.9rem;padding:0;border:0;background:none;
  font:inherit;font-size:.88rem;color:var(--ink-2);text-decoration:underline;
  text-underline-offset:.2em;cursor:pointer}
.iv__none:hover{color:var(--accent)}
.iv__none:disabled{opacity:.6;cursor:default}
@media(max-width:520px){.iv__s{flex-direction:column;align-items:flex-start;gap:.6rem}}
...
/* Shut by default. The summary is the whole row when closed, so it takes the
   click and shows a pointer; the marker is replaced with one that reads as an
   affordance rather than a bullet. An open row is lifted slightly so the one
   you are working in is obvious in a column of shut ones. */
.row + .row{margin-top:.6rem}
.row__sum{cursor:pointer;list-style:none;position:relative;padding-right:1.6rem}
.row__sum::-webkit-details-marker{display:none}
.row__sum::after{content:"+";position:absolute;right:.1rem;top:0;
  font-family:"IBM Plex Mono",monospace;font-size:1rem;color:var(--muted);line-height:1.2}
/* The minus character itself, not a CSS numeric escape. A backslash-2212 in
   here is an octal escape to JavaScript and the generator stops parsing —
   including when it is only being described in a comment, which is how this
   line was written twice. Same family as the backslashes halved on the way in:
   anything written into a file that writes a file has to survive the trip. */
details[open] > .row__sum::after{content:"−"}
.row__sum:focus-visible{outline:2px solid var(--accent);outline-offset:3px;border-radius:6px}
.row[open]{border-color:var(--accent-soft);box-shadow:var(--shadow)}
/* What a shut row still has to tell you. The scorecard spent weeks proving
   that a thing you cannot see is a thing nobody does, so every reason to open
   a row is printed on the outside of it. */
.row__flags{display:flex;flex-wrap:wrap;gap:.35rem;margin-top:.45rem}
.row__f{font-family:"IBM Plex Mono",monospace;font-size:.64rem;letter-spacing:.06em;
  text-transform:uppercase;font-weight:600;padding:.14rem .4rem;border-radius:4px;
  background:var(--surface-2);color:var(--muted);border:1px solid var(--line)}
.row__f--stop{background:#FDECEE;color:#A11B2B;border-color:#F2C6CC}
.row__f--warn{background:var(--signal);color:var(--signal-ink);border-color:var(--signal)}
.row__f--ok{background:#E9F7EF;color:#0B6B3A;border-color:#BFE3CE}
:root[data-theme="dark"] .row__f--stop{background:#2A1116;color:#FF9AA6;border-color:#4E2028}
:root[data-theme="dark"] .row__f--ok{background:#0D2419;color:#63D69B;border-color:#1D4430}
...
/* the times offered to an applicant, on her row */
.ivo{flex-basis:100%;display:flex;flex-direction:column;gap:.5rem;margin-top:.2rem;
  padding:.7rem .85rem;border:1px solid var(--line);border-radius:9px;background:var(--surface-2)}
.ivo__h{font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:.66rem;
  letter-spacing:.09em;text-transform:uppercase;color:var(--muted)}
.ivo__l{list-style:none;margin:0;padding:0;display:grid;gap:.35rem}
.ivo__s{display:flex;justify-content:space-between;align-items:center;gap:.8rem;
  padding:.45rem .6rem;border:1px solid var(--line);border-radius:7px;background:var(--surface)}
.ivo__s--pick{border-color:var(--accent);background:var(--accent-soft)}
.ivo__s--on{border-color:var(--good, #0B6B3A)}
.ivo__s b{font-size:.85rem;font-weight:700;display:block}
.ivo__st{display:block;font-size:.72rem;color:var(--muted);
  font-family:'IBM Plex Mono',ui-monospace,monospace;margin-top:.1rem}
.ivo__b{padding:.28rem .6rem;font-size:.76rem;white-space:nowrap}
.ivo__add{display:flex;gap:.4rem;align-items:center;flex-wrap:wrap}
.ivo__add input{font-family:inherit;font-size:.8rem;padding:.3rem .45rem;
  border:1px solid var(--line);border-radius:6px;background:var(--surface);color:var(--ink)}
.ivo__msg{margin:0;font-size:.8rem;color:var(--ink-2)}
.ivo__err{font-size:.78rem;color:#B3261E;min-height:1em}
:root[data-theme="dark"] .ivo__err{color:#F2B8B5}
...
.cal__nolink{color:var(--muted);font-style:italic}
...
.ivo__off{display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;margin-top:.5rem;padding-top:.6rem;border-top:1px solid var(--line)}
.ivo__off input[type="datetime-local"]{flex:0 0 auto;min-width:0}
.ivo__off input{flex:1;min-width:11rem;border:1px solid var(--line);border-radius:6px;padding:.4rem .6rem;font:inherit;font-size:.85rem;background:var(--surface);color:var(--ink)}
.rnav--sub{padding-left:2.1rem;font-size:.83rem}
.qempty{border:1px dashed var(--line);border-radius:10px;padding:1.6rem 1.2rem;text-align:center;color:var(--muted);font-size:.92rem;background:var(--surface-2)}
.qempty b{display:block;color:var(--ink-2);font-weight:600;margin-bottom:.2rem}
.rnav--sub.is-on{border-left-color:#fff}
.rnav--sub.is-empty{color:#61799C}
.rnav--sub.is-empty .rnav__n{opacity:.5}
.rnav--sub.is-on.is-empty{color:var(--accent-ink)}
...
/* Two of these can appear beside a verdict since 063, and only one of them is
   an instruction. What is still provisional is worth knowing; what is actually
   stopping her moving is worth acting on, so it keeps the loud colour. */
.sit__wait--soft{background:var(--surface-2);color:var(--muted);
  border:1px solid var(--line)}
...
/* An anchor when the proof is a link she pasted, a button when it is a
   screenshot that has to be signed first. Both have to look like the same
   thing, so the button is stripped back to the anchor rather than the anchor
   being dressed up as a button. */
/* The queue's stage headings. Quiet enough not to compete with the rows under
   them, loud enough to be a place rather than a caption — the mistake the
   interview scorecard made for weeks. */
.qgrp{margin-top:1.6rem}
.qgrp:first-child{margin-top:.4rem}
.qgrp__h{display:flex;align-items:center;gap:.55rem;margin:0 0 .7rem;
  font-family:"IBM Plex Mono",monospace;font-size:.7rem;letter-spacing:.12em;
  text-transform:uppercase;font-weight:600;color:var(--muted)}
.qgrp__h::after{content:"";flex:1;height:1px;background:var(--line)}
.qgrp__n{order:1;font-size:.7rem;padding:.1rem .42rem;border-radius:20px;
  background:var(--surface-2);color:var(--ink-2);border:1px solid var(--line)}

/* Where she joins. The pair beside Confirm, and the pair on a confirmed row. */
.ivo__cf{display:flex;gap:.4rem;align-items:center;flex-wrap:wrap}
.ivo__cf input[type=url]{font-family:inherit;font-size:.78rem;padding:.25rem .4rem;
  border:1px solid var(--line);border-radius:5px;background:var(--surface);color:var(--ink);
  min-width:11rem}
.ivo__link{margin-top:.5rem;padding-top:.5rem;border-top:1px dashed var(--line);
  display:grid;gap:.4rem}
.ivo__have{font-size:.8rem;color:var(--ink-2);overflow-wrap:anywhere}
.ivo__have b{font-family:"IBM Plex Mono",monospace;font-size:.66rem;letter-spacing:.08em;
  text-transform:uppercase;color:var(--muted);margin-right:.35rem}
.ivo__none{font-size:.8rem;color:var(--muted)}
.ivo__link .ivo__add input[type=url]{font-family:inherit;font-size:.78rem;padding:.25rem .4rem;
  border:1px solid var(--line);border-radius:5px;background:var(--surface);color:var(--ink);
  flex:1 1 14rem}
...
.sit__lnk[disabled]{cursor:default;opacity:.6}
...
function signDoc(path) {
  ...
  return fetch(storageBase() + "/object/sign/" + path, {
    ...
    /* storageBase(), not SB. Storage answers under /storage/v1 and the URL it
       hands back is relative to that, so pasting it onto the project host
       alone gives .../object/sign/... with the /storage/v1 missing — a 404,
       in a new tab, every time.

       The request three lines up was always built with storageBase(). Only
       the reply was not, so the two halves of the same function disagreed
       about where storage lives.

       This has been broken since 013 and could not be seen: opening a
       document requires a document, application_documents held none until
       today, and the panel has sat there the whole time with nothing in it.
       Same for 048's typing screenshot, which opens through here too. */
    return storageBase() + j.signedURL;
  });
  });
}
...
var STAGES = [
  ...
  ["interview",  "Interview",             "One interview with us on how you work, your setup and your connection."],
...
/* TYPE_TEXT, the passage she used to type here, went with the in-page test.
   The typing is taken at typingtest.com now and the proof is a screenshot, so
   there is no passage to hold and nothing in the page to measure it against.
   typingAccuracy() went the same way for the same reason. */
var TYPE_SITE = "https://www.typingtest.com/";

/* ── her interview, which she books herself ───────────────────────────────

   Moving somebody to Interview used to email her "we will be in touch to
   arrange it" and then give her nothing to do with that sentence. This is the
   something.

   Three states and nothing in between: times are offered and she has not
   answered, she has picked one and is waiting on us, or it is confirmed. A
   round she has declined shows as declined rather than as an empty card,
   because an empty card after she pressed a button reads as the button not
   having worked.

   Times are shown in her zone first and Central underneath. She is the one
   choosing, and a nine o'clock that turns out to be nine in the morning our
   time is the exact mistake this exists to prevent. */
function ivCard(a) {
  var slots = (a.slots || []).slice().sort(function (x, y) {
    return new Date(x.starts_at) - new Date(y.starts_at);
  });

  /* ── moved to interview before anybody offered her a time ────────────────

     031's interview mail says "Some times are waiting on your application
     page — pick whichever suits you", over a button reading Pick your time.
     It is sent by the stage change, and the stage can be changed before any
     slot exists — which is what happened the first time anybody used this.

     Returning "" here meant she followed that button to a page with no
     interview card on it at all. Not an empty one: nothing. The mail asserted
     something the page then declined to mention, and the only person who could
     tell it was a timing problem rather than a broken account was us.

     So the card exists from the moment she is at interview. It says the true
     thing — the times are coming and she does not owe us anything yet — which
     costs nothing when the slots arrive an hour later and is the difference
     between waiting and being ignored. */
  if (!slots.length) {
    if (a.status !== "interview") return "";
    return '<div class="card">' +
      '<div class="row__top"><span><span class="row__n">Your interview</span>' +
      '<span class="row__meta"> &middot; with us, camera on</span></span>' +
      '<span class="pill pill--assessment">Times coming</span></div>' +
      '<p class="msg" style="margin:1rem 0 0">One interview with us &mdash; how you work, and a ' +
      "check of your machine and connection. <b>We are picking times to offer you.</b> They will " +
      "appear here and you choose whichever suits you, so there is nothing to do until then " +
      "&mdash; if your email said times were waiting, they are on their way.</p></div>";
  }
  ...
  var live = [];
  for (var i = 0; i < slots.length; i++) {
    if (slots[i].confirmed_at) confirmed = slots[i];
    else if (slots[i].chosen_at) chosen = slots[i];
    if (!slots[i].confirmed_at) live.push(slots[i]);
  }
  ...
  var head =
    '<div class="row__top"><span><span class="row__n">Your interview</span>' +
    '<span class="row__meta"> &middot; ' + esc(String((confirmed || chosen || slots[0]).minutes || 30)) +
      " minutes, camera on</span></span>" +
    '<span class="pill pill--' + (confirmed ? "approved" : "assessment") + '">' +
  ...
  if (confirmed) {
    return '<div class="card">' + head +
      ...
      "</li></ol>" +
      '<p class="msg" style="margin-top:.9rem">If you need to move it, tell us and we will ' +
      "offer new times.</p></div>";
  }

  if (declined) {
    return '<div class="card">' + head +
      '<p class="msg" style="margin-top:1rem">You told us none of those worked. We will send ' +
  ...
  return '<div class="card">' + head +
    ...
    (chosen
      ? '<p class="msg" style="margin-top:.9rem">Picked a different one? Choose it and we will ' +
        "take the last one back.</p>"
      : '<button class="iv__none" type="button" data-none="' + esc(a.id) + '">' +
        "None of these work for me</button>") +
    '<p class="msg msg--bad" id="iv-err" style="display:none"></p>' +
    "</div>";
}

/* The two things she can press. Both go through a function rather than a
   write: the page is granted no insert or update on interview_slots at all,
   so a browser that lies about which slot is hers is refused by the grant
...
function wireIv(a) {
  ...
  Array.prototype.forEach.call(document.querySelectorAll(".iv__go"), function (b) {
    b.addEventListener("click", function () {
      if (busy) return;
      busy = true;
      b.disabled = true;
  ...
  var none = document.querySelector(".iv__none");
  if (none) {
...
function assessCard(a, s) {
  ...
  /* ── a written part marked done with nothing in it ───────────────────────

     close_part stamps part_done for whichever part it is told about and never
     looks at what it is stamping, so an accidental press one second after the
     part opened reads here as finished — the same tick, the same word, as
     twenty minutes of work. It is not finished in any sense she would
     recognise, and while the twenty minutes are still running there is
     nothing stopping her writing the reply after all: close_part returns the
     original timestamp when a part is closed twice, so finishing it properly
     writes the text and leaves the record of when she first stopped alone.

     So the row goes back to being unfinished. Everything else follows from
     that on its own — part() reads the clock for anything not done, which
     puts a Resume on it and says how long is left — and once the time has
     gone it is done for real, and says what it holds instead of "done". */
  var writtenEmpty = !!fin.written && !((s && s.written_reply) || "").trim();
  if (writtenEmpty && msLeft("written") > 0) done.written = false;
  ...
  /* dim is for a note that reports something missing rather than something
     achieved. "16 wpm, we check it" and "nothing written" are both notes, and
     printing them in the same confident accent is how an empty work sample
     reaches the person marking it wearing the colour of a finished one. */
  function part(k, n, t, d, isDone, note, dim) {
    ...
    return '<li class="apt' + (isDone ? " is-done" : "") + '">' +
      ...
      "</span></span>" +
      (isDone
        ? '<span class="apt__s' + (dim ? " apt__s--none" : "") + '">' +
          esc(note || "done") + "</span>"
  ...
  return '<div class="card">' +
    ...
    '<ol class="apts">' +
      ...
      part("written", wantsSales ? 5 : 4, "Written reply to a customer",
        "Twenty minutes, about 150 words" + (writtenEmpty ? " &middot; nothing written yet" : ""),
        done.written, writtenEmpty ? "nothing written" : null, writtenEmpty) +
      part("typing", wantsSales ? 6 : 5, "Typing and your setup",
        "A typing test on another site, a screenshot of it, and a connection check",
...
/* The typing test, on typingtest.com, with a screenshot as the proof.

   It has been in both places now. 045 built it into this page; 048 moved it
   off, because score_typing is worked out from typing_wpm and typing_accuracy,
   those columns are granted to the applicant, and anybody who can open a
   browser console can send whatever numbers they like — which on the Admin
   Tasks track walked an application to Interview on a made-up figure. 481c5df
   brought it back on the grounds that opening two links per applicant is a job
   that does not survive the first dozen of them.

   Both of those are true, and the second one is a cost rather than a
   correction. This is that cost being accepted deliberately: she takes the
   test somewhere that will show her a result, sends the result as a picture,
   and somebody here opens the picture and writes down what it says.

   What it buys is the thing 048 was built around — the two figures on the row
   mean different things, and the scoring uses the checked pair:

     typing_wpm, typing_accuracy                    what she says she scored
     typing_verified_wpm, typing_verified_accuracy  what somebody here read

   So nothing reaches a verdict on its own any more. That is not a side effect
   to be fixed later, it is the point, and /admin has had the panel for it
   since 048 with nothing to open.

   Worth being honest about what a screenshot is: proof that a run happened,
   not that she ran it. Same class of thing as the paste guard on the written
   reply — a speed bump, not a wall — and the real check is still the
   interviewer asking her about her own answers.

   The connection speed test is untouched. It stays a link, because it
   genuinely cannot be measured from in here, and it is the thing that stops
   somebody starting a client on a line that cannot hold a call. */
function typingPart() {
  /* Her own folder in applicant-docs, which is where her CV already is. The
     bucket takes png and jpeg from a signed-in applicant into the folder named
     after her application and nothing else, so this needed no new policy — and
     047 caps that folder at five files, which a CV plus a screenshot per
     re-sit stays inside. */
  var appId = (SIT && SIT.application_id) || "";
  var shot = null;                                 /* chosen this visit */
  var shotPath = (SIT && SIT.typing_proof) || "";  /* one already sent */

  partShell("Typing and accuracy", 0,
    '<p class="msg" style="margin:1rem 0">Take a one-minute typing test at typingtest.com, ' +
      "then come back to this page with your result. Nothing here is timed, so take it as many " +
      "times as you like and give us the run you are happy with.</p>" +

    '<p style="margin:1.1rem 0"><a class="btn btn--ghost" id="a-site" target="_blank" ' +
      'rel="noopener noreferrer" href="' + TYPE_SITE + '">Take the test at typingtest.com ' +
      "&nearr;</a></p>" +

    '<label class="a-lbl" for="a-shot">Screenshot of your result</label>' +
    '<input id="a-shot" type="file" accept="image/png,image/jpeg" ' +
      'style="width:100%;margin:.4rem 0 .3rem">' +
    '<p class="msg" id="a-shot-s" style="margin:.3rem 0 1.3rem">' +
      (shotPath ? "A screenshot is already attached. Choose another to replace it."
                : "The whole result page, showing your words a minute and your accuracy. A photo " +
                  "of the screen is fine as long as both numbers can be read.") + "</p>" +

    '<div style="display:flex;flex-wrap:wrap;gap:.9rem">' +
      '<div style="flex:1 1 11rem">' +
        '<label class="a-lbl" for="a-wpm">Words a minute</label>' +
        '<input id="a-wpm" type="number" min="0" max="250" style="width:100%;margin-top:.4rem" ' +
          'value="' + (SIT && SIT.typing_wpm != null ? SIT.typing_wpm : "") + '">' +
      "</div>" +
      '<div style="flex:1 1 11rem">' +
        '<label class="a-lbl" for="a-acc">Accuracy</label>' +
        '<input id="a-acc" type="number" min="0" max="100" style="width:100%;margin-top:.4rem" ' +
          'value="' + (SIT && SIT.typing_accuracy != null ? SIT.typing_accuracy : "") + '">' +
      "</div>" +
    "</div>" +
    '<p class="msg" style="margin:.5rem 0 1.3rem">Both are printed on the result page. Type them ' +
      "in exactly as they appear &mdash; we check them against your screenshot.</p>" +

    '<label class="a-lbl" for="a-conn">Link to a speed test of your connection</label>' +
    '<input id="a-conn" type="url" maxlength="500" ' +
      'style="width:100%;margin:.4rem 0 .3rem" placeholder="https://" value="' +
      esc((SIT && SIT.connection_proof) || "") + '">' +
  ...
  var file = document.getElementById("a-shot");
  var shotMsg = document.getElementById("a-shot-s");
  var wpm = document.getElementById("a-wpm");
  var acc = document.getElementById("a-acc");
  var conn = document.getElementById("a-conn");

  file.addEventListener("change", function () {
    shot = (file.files && file.files[0]) || null;
    if (!shot) {
      shotMsg.textContent = shotPath
        ? "A screenshot is already attached. Choose another to replace it."
        : "No screenshot chosen yet.";
      return;
    }
    shotMsg.textContent = shot.name + " — " + kb(shot.size) + ", not sent yet.";
  });
  ...
  document.getElementById("a-done").addEventListener("click", function () {
    ...
    /* Everything is read and refused before anything is uploaded, so a number
       she has to fix does not cost her a round trip and leave a file in the
       bucket that nothing points at. */
    var w = String(wpm.value || "").trim();
    var a = String(acc.value || "").trim();
    if (w === "" || !(Number(w) >= 0 && Number(w) <= 250)) {
      return fail("Put in the words a minute from your result page — a number up to 250.", wpm);
    }
    ...
    done.textContent = shot ? "Sending your screenshot…" : "Saving…";

    /* The upload first, the row second. A row naming a file that never arrived
       is the one order that lies; a file with no row yet is just a file, and
    ...
    (shot ? sendShot(shot) : Promise.resolve(shotPath)).then(function (path) {
      ...
      fail(e && e.message === "signed out"
        ? "You have been signed out. Sign in again — the rest of this is still here."
        : "That screenshot did not send. Check your connection and try again.", null);
    });
  });

  /* Into the bucket, then into the table the document lists are built from.
     Both pages read application_documents rather than listing storage, so a
     file that skips the table is invisible to her and to whoever is deciding
     on her — which is exactly how a CV went missing once already. */
  function sendShot(f) {
    if (!appId) return Promise.reject(new Error("no application"));
    var name = "typing-" + Date.now() + (f.type === "image/png" ? ".png" : ".jpg");
    var path = appId + "/" + name;
    return liveSession().then(function (sess) {
...
function writtenPart() {
  ...
  box.addEventListener("input", function () {
    ...
    /* The refusal below is about a paste, and the paste is over. Typing is
       the thing it asked for, so the first keystroke answers it — otherwise
       somebody tries to paste once, is correctly refused, writes the whole
       hundred and fifty words by hand, and is still being told off for
       pasting when they press Done. Found doing exactly that.

       Only this message is cleared. #a-err is shared with closePart, and a
       save that failed is not something a keystroke resolves — clearing that
       on the next letter typed would hide the one error here worth seeing. */
    var pasteWarn = document.getElementById("a-err");
    if (pasteWarn && pasteWarn.textContent.indexOf("pasting is turned off") > -1) {
      pasteWarn.textContent = "";
      pasteWarn.style.display = "none";
    }

    /* And the empty-box question is answered by writing something, so the
       button pair goes back to being one button the moment she does. */
    if (box.value.trim()) resetConfirm();
  });
  ...
  /* ── closing this part with nothing in it ────────────────────────────────

     Pressing Done on an empty box used to close the part, and the card has no
     way back into a finished one — so one misclick a second after opening it
     ended the work sample, silently and for good. It happened: the written
     part on the only assessment in the database was opened at 01:57:01 and
     marked done at 01:57:02 with nought characters in it.

     This asks rather than refuses. Somebody who genuinely wants to give up on
     the part may, and this project has been hurt more by controls that refuse
     legitimate things than by ones that ask. */
  var doneBtn = document.getElementById("a-done");
  var keepBtn = null;

  function resetConfirm() {
    if (!keepBtn) return;
    keepBtn.parentNode.removeChild(keepBtn);
    keepBtn = null;
    doneBtn.textContent = "Done writing";
    doneBtn.className = "btn btn--solid";
    var w = document.getElementById("a-err");
    if (w && w.textContent.indexOf("not written anything") > -1) {
      w.textContent = "";
  ...
  doneBtn.addEventListener("click", function (ev) {
    ...
    /* The deadline closes this part by clicking this very button — partShell's
       tick calls done.click() when the clock passes zero, which is how twenty
       minutes of writing gets banked rather than lost with the tab. That path
       must never be stopped by a question nobody is there to answer.

       isTrusted is the line: false on a scripted click, true on a real press.
       So expiry closes straight through exactly as it did before, and only a
       person pressing the button is ever asked. */
    if (ev && ev.isTrusted && !keepBtn && !box.value.trim()) {
      var w = document.getElementById("a-err");
      if (w) {
        w.style.display = "";
        w.textContent = "You have not written anything yet. A person reads this part, " +
          "and closing it empty leaves them nothing to read.";
      }
      doneBtn.textContent = "Close it empty";
      doneBtn.className = "btn btn--ghost";
      keepBtn = document.createElement("button");
      keepBtn.type = "button";
      keepBtn.className = "btn btn--solid";
      keepBtn.style.cssText = "margin-top:1.1rem;margin-right:.6rem";
      keepBtn.textContent = "Keep writing";
      keepBtn.addEventListener("click", function () { resetConfirm(); box.focus(); });
      doneBtn.parentNode.insertBefore(keepBtn, doneBtn);
      return;
    }
...
function loadApplications() {
  ...
  Promise.all([
    ...
    api("application_assessment?select=application_id,track,attempt,started_at,submitted_at," +
      ...
      .catch(function () { return []; }),
    api("interview_slots?select=id,application_id,starts_at,minutes,chosen_at," +
        "confirmed_at,declined_at,meeting_url&order=starts_at")
      .catch(function () { return []; })
  ])
    .then(function (r) {
      ...
      var slotsBy = {};
      (r[3] || []).forEach(function (v) {
        if (!v.application_id) return;
        (slotsBy[v.application_id] = slotsBy[v.application_id] || []).push(v);
      });
      rows.forEach(function (a) {
        a.docs = byId[a.id] || [];
        a.sit = sits[a.id] || null;
        a.slots = slotsBy[a.id] || [];
      });
      render(user, rows);
    })
    .catch(function (e) {
...
var IV_CONVERSATION = [
  ...
  ["iv_tools", "Using the tools",
   "Inbox and calendar, sheets, a helpdesk or CRM, a chat app. Learning a client's, not knowing yours."],
...
/* Every column the scorecard writes, in one place. The queue view averages
   exactly these into iv_avg, and the page has to agree with it: two lists of
   score columns is how a scored interview goes on counting as unscored. */
var IV_COLS = IV_CONVERSATION.map(function (k) { return k[0]; })
  .concat(Object.keys(IV_JOBS).map(function (k) { return IV_JOBS[k]; }));

/* Scored at all, under either scorecard. 008's five and 065's eight both
   count, because a row scored in August is still a row somebody scored. */
function hasScore(a) {
  var iv = a.iv_avg, old = a.score_avg;
  return (iv !== null && iv !== undefined && iv !== "") ||
...
/* ── the times we offer her ───────────────────────────────────────────────

   The date box beside this still works and still writes interview_at directly,
   which is how it has always been done and how it should stay for a time
   agreed on the phone. This is the other way: offer two or three, let her pick
   one, confirm it.

   Times are shown in Central, because this is our side of the screen. Hers are
   shown to her in her own zone, with Central underneath, on /status.

   Nothing here is a write. Every button calls one of the functions in 062, and
...
function ivOffer(btn) {
  ...
  var err = wrap ? wrap.querySelector("[data-iverr]") : null;
...
/* One shape for all three: disable, call, reload, and put the reason on the
   row rather than in a dialog. The functions in 062 raise sentences meant to
   be read — "she has not picked that one", "that one is confirmed" — so the
   message is shown as it comes back instead of being replaced with a generic
   one that says less. */
/* One shape for all three: disable, call, refetch, redraw the row.

   Refetched rather than guessed at locally, the same rule addNote follows a
   few functions up. Confirming writes application_tracking.interview_at as
   well as the slot, so the row is read back from the queue too — inventing
   that date here would put a different one on the screen from the one in the
   database, and the Interviews tab reads the database.

   The reason is shown as it comes back. The functions in 062 raise sentences
   meant to be read — "she has not picked that one", "that one is confirmed
   — cancel it with her, not from here" — and replacing those with a generic
   failure would say less than the database already did. */
function ivAct(btn, fn, body, saying, errEl) {
  var row = btn.closest(".row");
  var id = row ? row.getAttribute("data-id") : null;
  var wrap = btn.closest(".ivo");
  var err = errEl || (wrap ? wrap.querySelector("[data-iverr]") : null);
  var was = btn.textContent;
  btn.disabled = true;
    ...
    .then(function () {
      return Promise.all([
        api("interview_slots?select=id,application_id,starts_at,minutes,offered_by," +
            "chosen_at,confirmed_at,declined_at,meeting_url&application_id=eq." +
            encodeURIComponent(id) + "&order=starts_at"),
        api("application_queue?select=*&id=eq." + encodeURIComponent(id))
      ]);
    })
    .then(function (r) {
      ...
      rec.slots = r[0] || [];
      var fresh = (r[1] || [])[0];
      if (fresh) rec.interview_at = fresh.interview_at;
      var tmp = document.createElement("div");
      tmp.innerHTML = rowHtml(rec);
      /* Open state travels with the node, so it has to be carried over — you
         are always inside a row when you press these. */
      if (row.open) tmp.firstChild.open = true;
      row.replaceWith(tmp.firstChild);
      /* The tab beside this one counts interviews with no date, interviews
         nobody scored and now interviews booked before the assessment. A
         confirm changes all three inputs, so it is redrawn rather than left
         showing the count from before the click. */
      drawCalendar();
    })
    .catch(function (e) {
      btn.disabled = false;
      btn.textContent = was;
      /* Through why(), which unwraps PostgREST's body to the sentence inside.

         This used to print e.message, which IS the whole body, so every one of
         these functions' carefully written refusals reached the screen as

           {"code":"P0001","details":null,"hint":null,
            "message":"that is the time it is already at"}

         The comment further up already said these were sentences meant to be
         read, and had said so since 062. Nothing read them. why() has existed
         the whole time and is called from thirty-odd other places; this was
         one of the few that did not. */
      if (err) err.textContent = why(e);
    });
}
/* Moving a booked interview to a time we have decided.

   One press, not two. The date box is the confirmation: an empty one does
   nothing and says so, and nobody fills in a date and time by accident. That
   is the difference from cancelling, which is one click away from a mistake
   somebody cannot take back.

   The reason box below is shared with cancelling and read by both, so a move
   can be explained in the notes the same way. */
function ivResched(btn) {
  var wrap = btn.closest(".ivo");
  var at = wrap ? wrap.querySelector("[data-ivmove]") : null;
  var err = wrap ? wrap.querySelector("[data-iverr]") : null;
  var say = function (m) { if (err) err.textContent = m; };
  say("");
...
/* Calling off a booked interview.

   Two presses, for the same reason the written part learned to ask: this
   mails her, drops the booking and cannot be undone, and one stray click on
   a row somebody was reading is not consent. The first press only arms it,
   and says what the second one will do.

   The reason goes to the note log and never into her email. She is told the
   interview is off and that new times are coming, which is what she needs;
   "clash with a client call" is for whoever opens her row in three weeks. */
function ivCancel(btn) {
  var wrap = btn.closest(".ivo");
  var err = wrap ? wrap.querySelector("[data-iverr]") : null;

  if (btn.getAttribute("data-armed") !== "1") {
    btn.setAttribute("data-armed", "1");
    btn.textContent = "Press again to cancel it";
    btn.className = "btn btn--solid ivo__b";
    if (err) {
      err.textContent = "This emails her that it is off, takes the time off the books " +
        "and frees her to be offered another. There is no undo.";
    }
    return;
  }

  var why = wrap ? wrap.querySelector("[data-ivwhy]") : null;
  var text = why && why.value ? why.value.slice(0, 500) : null;
  ivAct(btn, "rpc/cancel_application_interview",
    { slot: btn.getAttribute("data-ivcancel"), why: text },
    "Cancelling…", err);
}

function ivOffered(a) {
  var slots = (a.slots || []).slice().sort(function (x, y) {
    return new Date(x.starts_at) - new Date(y.starts_at);
  });

  var confirmed = null;
  for (var i = 0; i < slots.length; i++) if (slots[i].confirmed_at) confirmed = slots[i];

  var declined = !confirmed && slots.length &&
    slots.every(function (v) { return !!v.declined_at; });

  var list = slots.map(function (v) {
    var when = slotDay(v.starts_at, CENTRAL) + " · " + slotClock(v.starts_at, CENTRAL);
    var state = v.confirmed_at ? "confirmed"
      : v.declined_at ? "she said no"
      : v.chosen_at ? "she picked this" : "offered";
    return '<li class="ivo__s' + (v.chosen_at && !v.confirmed_at ? " ivo__s--pick" : "") +
      ...
      (confirmed
        ...
        : v.chosen_at
          ? '<span class="ivo__cf">' +
            '<input type="url" data-ivurl maxlength="500" placeholder="Joining link, optional" ' +
            'aria-label="Where she joins, if you have it yet">' +
            '<button class="btn btn--solid ivo__b" type="button" data-ivconfirm="' + esc(v.id) + '">Confirm</button>' +
            "</span>"
          : '<button class="btn btn--ghost ivo__b" type="button" data-ivdrop="' + esc(v.id) + '">Withdraw</button>') +
      "</li>";
  }).join("");
  ...
  return '<div class="ivo">' +
    ...
    (declined
      ...
      : "") +
    /* Where she joins. confirm_application_interview has taken a url since 062
       and this page never passed one, so meeting_url was null on every
       confirmed interview and her mail always fell back to "we will send the
       joining details before the day" — which nothing then sent.

       Two places to put it, because the link and the decision do not arrive
       together. Usually you agree the time first and make the room afterwards,
       so it can go in with Confirm OR onto a confirmed row later, and adding
    ...
    (confirmed
      ? '<div class="ivo__link">' +
          ...
          '<span class="ivo__add">' +
            ...
            '<button class="btn btn--ghost ivo__b" type="button" data-ivlink="' + esc(confirmed.id) + '">' +
            (confirmed.meeting_url ? "Replace" : "Send link") + "</button>" +
          "</span>" +
        "</div>" +
        '<p class="ivo__msg">Confirmed, and the date is on her row. Withdraw is off ' +
        "&mdash; a booked time is changed with her, not behind her.</p>" +
        /* And then, once it has been. Until 068 the sentence above was the end
           of the road: after the conversation there was nothing that could
           take the time off the books, offer another, or stop her seeing a
           Join button for a meeting nobody would be at. */
        /* Moving it, which is the ordinary case and so comes first.

           The conversation almost always ends with a time already agreed —
           "Wednesday is no good, can we do Thursday at eleven" — and at that
           point offering her a fresh set to choose from asks a question she
           has already answered, and puts her interview back into an undecided
           state while she replies. Cancelling is for when there is no new time
        ...
        "</div>" +
        '<div class="ivo__off">' +
          '<input type="text" data-ivwhy maxlength="500" ' +
            'placeholder="Why, in a few words &mdash; for the notes, not for her" ' +
            'aria-label="Why this interview is being cancelled">' +
          '<button class="btn btn--ghost ivo__b" type="button" data-ivcancel="' +
...
function rowHtml(a) {
  ...
  /* ── shut by default, and the summary has to earn that ──────────────────

     A row is about two screens tall: the working style, the scorecard, the
     socials, the stage controls, the times offered and the notes. Four
     applicants is fine. Forty is a scroll nobody reads, and the queue is
     meant to be read.

     So it collapses. The whole risk of that is the one the interview
     scorecard demonstrated for weeks — a shut thing is an invisible thing —
     so the summary carries every reason you would have opened it: what stage
     she is at, whether she has been waiting, whether she has been graded, and
     whether her interview is booked. Nothing that needs doing is only visible
     inside.

     A <details> rather than a class toggle, so it works before any script
     runs, keyboard opens it, and the browser's own find-in-page can reach the
     text inside a shut one. */
  var flags = "";
  if (a.is_ghosted) flags += '<span class="row__f row__f--stop">no reply</span>';
  if (a.interview_unresolved) flags += '<span class="row__f row__f--warn">interview not written up</span>';
  if (a.sit && a.sit.submitted_at && a.sit.verdict === "passed") {
    flags += '<span class="row__f row__f--ok">passed</span>';
  } else if (a.sit && a.sit.submitted_at && a.sit.verdict === "below_line") {
    flags += '<span class="row__f row__f--stop">below the line</span>';
  } else if (a.status === "assessment" && (!a.sit || !a.sit.submitted_at)) {
  ...
  return (
    '<details class="row" data-id="' + esc(a.id) + '">' +
      ...
      (flags ? '<div class="row__flags">' + flags + "</div>" : "") +
      "</summary>" +
      skillLine(a) +
      discLine(a) +
      sitLine(a) +
...
/* ── the assessment, which nothing in here used to show ──────────────────
   ...
   Two boxes on it matter, and what they mean changed in 063.

   They used to be the gate: until somebody typed what the typing proof said
   and what the writing was worth, 049 held the verdict at 'in_progress'. So
   this panel showed "waiting on ..." INSTEAD of a verdict, which was right,
   because there was no verdict to show.

   063 grades on the send. There is always a verdict now, and hiding it behind
   the same "waiting on ..." line would have meant making the grading automatic
   and still not being able to see the grade. So the verdict is always shown,
   and the two boxes say what is still provisional about it:

     the typing not checked   she is graded on a number she typed about
                              herself, and 063's advance_on_assessment will
                              not move her to Interview until somebody reads
                              the proof — so a 'passed' row sitting at
                              assessment is this, not a bug

     the writing not marked   english is scored on the multiple choice half
                              alone until then, and marking it re-scores the
                              row and can change the verdict either way */
function sitLine(a) {
  var s = a.sit;
  if (!s || !s.submitted_at) return "";

  var needTyping = s.typing_verified_wpm === null || s.typing_verified_wpm === undefined;
  var need = [];
  if (needTyping) need.push("the typing checked");
  ...
  return '<div class="sit" data-sit="' + esc(a.id) + '">' +
    '<div class="sit__hd"><b>Assessment</b>' +
      ...
      /* After the verdict, not instead of it. The verdict is the answer; this
         says how much of it is still somebody's word for it. */
      (need.length
        ? '<span class="sit__wait sit__wait--soft">on her own figures &middot; ' +
          esc(need.join(" and ")) + " still to do</span>"
        : "") +
      /* The one consequence somebody would otherwise have to work out from a
         stage that has not moved. */
      (s.verdict === "passed" && needTyping
        ? '<span class="sit__wait">not invited until the typing is checked</span>'
        : "") +
    "</div>" +
    ...
    '<div class="sit__row">' +
      ...
      /* Two shapes of proof, because there have been two arrangements. Since
         the typing moved back to typingtest.com the proof is a screenshot in
         applicant-docs, and a private bucket is not something an href reaches
         — it needs signing for a minute, the same way her CV is opened two
...
function drawCalendar() {
  ...
  /* Computed here rather than read off interview_unresolved, for two reasons.
     The view's flag is right — 065 widened it to both scorecards deliberately
     — but it only refreshes on a load, so scoring somebody left them sitting
     in Needs attention until a reload. And the clause that used to sit beside
     it tested score_avg alone, which under 065 is null however carefully the
     interview was scored, so a scored interview never stopped being flagged. */
  var unresolved = past.filter(function (a) {
    return a.pipeline === "interviewed" && !hasScore(a);
  });
  var atInterviewNoDate = ALL.filter(function (a) {
    var live = (a.slots || []).filter(function (v) { return !v.declined_at; });
    if (a.interview_at || live.length) return false;
    return a.pipeline === "interviewed" || a.status === "interview";
  });
  ...
  /* She said none of the times work.

     decline_application_interviews marks them all declined and tells nobody —
     it is a database function, it has no inbox. Without this the most active
     thing an applicant can do in this flow is also the most invisible: she
     answers within the hour, and the answer sits in a column until somebody
     opens her row for an unrelated reason.

     Read off the slots rather than off a stage, because declining does not
     move her stage — she is still at interview, still waiting, and that is
     exactly why nothing else would notice. */
  var saidNo = ALL.filter(function (a) {
    var v = a.slots || [];
    if (!v.length) return false;
    if (v.some(function (x) { return x.confirmed_at; })) return false;
    return v.every(function (x) { return x.declined_at; });
  });

  var badge = document.getElementById("tab-cal");
  var problems = unresolved.length + clashes.length + atInterviewNoDate.length +
    beforeAssessment.length + saidNo.length;
    ...
    issues =
      '<div class="cal__issues">' +
        ...
        (atInterviewNoDate.length
          ? '<p class="cal__i"><b>' + atInterviewNoDate.length + " at interview with nothing arranged.</b> " +
            "No date, and no times offered for them to pick from: " +
          ...
          : "") +
        (saidNo.length
          ? '<p class="cal__i"><b>' + saidNo.length +
            " said none of the times work.</b> " +
            "Waiting on a new set from you: " +
            saidNo.slice(0, 6).map(function (a) { return esc(a.name || a.email); }).join(", ") +
            (saidNo.length > 6 ? " and " + (saidNo.length - 6) + " more" : "") + "</p>"
          : "") +
  ...
  if (!booked.length && !atInterviewNoDate.length && !saidNo.length) {
        ...
        days[key].map(function (a) {
          return '<div class="cal__row">' +
            ...
            '<span class="cal__w">' + esc((a.tracks || []).join(" + ")) +
              (a.interviewer ? " &middot; " + esc(a.interviewer) : "") +
              /* Where to join, on the screen you open to join.

                 This is the page you look at on the morning of an interview,
                 and the link was three clicks away in another tab: back to
                 Applications, find her row, open it, scroll to the panel. The
                 link is on the confirmed slot and the slot is already on the
                 row, so there was nothing to fetch — only nobody had asked
                 for it here.

                 A booking with no link says so, because five minutes before
...
function wireTabs() {
  ...
  bar.addEventListener("click", function (e) {
    ...
    /* The queue's own entries carry a stage as well as a pane. Applications
       carries an empty one, which is the overview rather than a missing
       value, so the test is against null and never against truthiness. */
...
function shownRows() {
  ...
  var shown = ALL.filter(function (a) {
    ...
    if (st === "__late") {
      ...
      /* Either scorecard counts. Testing score_avg alone put every interview
         scored under 065 back into "not yet scored". */
      if (a.pipeline !== "interviewed" || hasScore(a)) return false;
...
/* ── the queue, in stages rather than one list ────────────────────────────
   ...
   It was one flat list ordered by who had waited longest, with a pipeline
   filter above it. That is the right order inside a stage and the wrong shape
   across them: somebody who applied this morning, somebody sitting exams and
   somebody with an interview booked need different things done, and they were
   interleaved by age with nothing but a small pill to tell them apart.

   Every stage is listed in ladder order, and only the ones holding somebody
   are drawn — an empty heading is a shelf with nothing on it. The count is on
   the heading because "Assessment 1" answers the question the filter was being
   used to ask.

   declined is last and separate on purpose: it is not a step on the way
   anywhere, and putting it after hired keeps the ladder reading downward.

   A status this list does not know about still appears, under its own name,
   rather than vanishing — the failure mode of grouping is a row nobody can
   find, and a row nobody can find is worse than a row in the wrong place. */
var QUEUE_ORDER = ["applied", "assessment", "interview", "approved", "hired", "declined"];
var QUEUE_HEAD = {
  applied:    "New applicants",
  assessment: "Assessment",
  interview:  "Interview",
  approved:   "Approved — in training",
  hired:      "Hired",
  declined:   "Declined"
};

/* Which stage the rail is standing on. "" is the overview, which carries the
   tiles and the Breakdown because those count the whole queue and belong to
   none of the stages. */
var STAGE = "";

/* Whether anything is narrowing the queue right now. It decides one thing:
   a search has to reach people at every stage, or the queue becomes a place
   where you must already know somebody’s stage in order to find them,
   which is worse than the scroll this replaced. */
function narrowing() {
  var ids = ["q", "filter", "fskill", "flevel", "fdate"];
...
/* The counts beside each rail entry, and an entry for any status this list
   has never heard of. Counted off ALL rather than off what is shown, because
   a count that moves when you type into the search box is not a count of
   anything — it is the filter, reported twice. */
function paintStageCounts() {
  var by = {};
  var extra = [];
  ALL.forEach(function (a) {
    var k = a.status || "applied";
    if (QUEUE_ORDER.indexOf(k) < 0 && extra.indexOf(k) < 0) extra.push(k);
    by[k] = (by[k] || 0) + 1;
  });

  QUEUE_ORDER.forEach(function (k) {
    var el = document.getElementById("rn-" + k);
    if (!el) return;
    var n = by[k] || 0;
    el.textContent = String(n);
    var btn = el.closest("[data-stage]");
    if (btn) btn.classList.toggle("is-empty", n === 0);
  });

  var slot = document.getElementById("rn-extra");
  if (slot) {
    slot.innerHTML = extra.map(function (k) {
      return '<button class="rnav rnav--sub" data-tab="apps" data-stage="' + esc(k) +
        '" type="button">' + esc(LABEL[k] || k) +
        '<span class="rnav__n">' + (by[k] || 0) + "</span></button>";
    }).join("");
  }

  var apps = document.getElementById("tab-apps");
  if (apps) apps.textContent = ALL.length ? String(ALL.length) : "";
}

/* The first stage with anybody in it, so the queue never opens on an empty
   page. Falls back to the overview when nobody has applied at all. */
function firstBusyStage() {
  for (var i = 0; i < QUEUE_ORDER.length; i++) {
    var k = QUEUE_ORDER[i];
    for (var j = 0; j < ALL.length; j++) {
      if ((ALL[j].status || "applied") === k) return k;
    }
  }
  return "";
}

function grouped(shown, heads) {
  var by = {};
  var extra = [];
  ...
  return QUEUE_ORDER.concat(extra).map(function (k) {
    var group = by[k];
    if (!group || !group.length) return "";
    /* No heading when the rail already names the stage you are standing on.
       Printing it again is the page repeating the navigation back at you. */
    if (heads === false) return group.map(rowHtml).join("");
    return '<div class="qgrp">' +
      '<h3 class="qgrp__h">' + esc(QUEUE_HEAD[k] || LABEL[k] || k) +
      '<span class="qgrp__n">' + group.length + "</span></h3>" +
...
function paint() {
  ...
  /* Three states, and only one of them draws the whole queue.

     Overview  the tiles and the Breakdown, which count everybody and so
               belong to no single stage. No rows.
     A stage   that stage's rows only, no headings, no tiles.
     Narrowed  a search reaches every stage, with the headings back on so each
               hit says where its person is standing.

     The tiles are hidden rather than left up on a stage page: whole-queue
     figures above a single stage's rows is the confusion this replaced. */
  var narrowed = narrowing();
  var overview = !narrowed && STAGE === "";
  var kpis = document.getElementById("kpis");
  var stats = document.getElementById("stats-card");
  if (kpis) kpis.hidden = !overview;
  if (stats) stats.hidden = !overview;
  ...
  if (here.length) {
    ...
    rows.innerHTML = '<p class="msg">Nothing matches that.</p>';
  } else {
    /* An empty stage says so. It used to be drawn as nothing at all, which
       made "nobody is here" and "you have scrolled past it" identical. */
    rows.innerHTML = '<div class="qempty"><b>Nobody is at ' +
      esc((QUEUE_HEAD[STAGE] || LABEL[STAGE] || STAGE).toLowerCase()) + ".</b> " +
      "When somebody reaches this stage they appear here.</div>";
  }
...
function save(row) {
     ...
     Whether anything is sent is decided by the changed flag alone. */
  {
    ...
    if (changed) {
      jobs.push(api("application_tracking", {
        ...
        /* Same trick for who scored the interview, and for the same reason.
           The database stamps scored_by from the token whenever a score moves,
        ...
        /* Every score column that was sent, read off the row rather than off
           a list. It used to walk SKILLS and rewrite the five score_ columns
           008 defined, so the eight iv_ columns 065 added were saved to the
           database and never put back on the local row.

           The row is redrawn from that row immediately afterwards, so the
           scores you had just typed were read back from a stale object and
           the selects went to blank. The save had worked. The screen said it
           had not, which is the one thing a screen must not say. */
        row.querySelectorAll("[data-score]").forEach(function (el) {
          var col = el.getAttribute("data-score");
  ...
  Promise.all(jobs).then(function () {
    ...
    if (rec) {
      ...
      var mean = function (cols) {
        var got = cols.map(function (c) { return rec[c]; })
      ...
      rec.score_avg = mean(SKILLS.map(function (k) { return k[0].replace("skill_", "score_"); }));
      /* And the one the header actually prints. Only score_avg was recomputed
         here, so the average above a freshly scored interview stayed at
         whatever it was before — blank, the first time anybody scored one.
         The comment above says a stale header reads as a bug. It was right. */
      rec.iv_avg = mean(IV_COLS);
      var fresh = rowHtml(rec);
      var tmp = document.createElement("div");
      tmp.innerHTML = fresh;
      var next = tmp.firstChild;
      /* The row is a <details> now, so its open state lives on the node and
         goes with the node it replaces. Carried across, or every save shuts
         the row you were working in and you reopen it to see whether the save
         worked. */
      if (row.open) next.open = true;
      row.replaceWith(next);
...
function render(email, apps, notes, socials, docs, disc, sits, slots) {
  var byId = {};
  ...
  (slots || []).forEach(function (v) {
    if (!v.application_id) return;
    (slotsById[v.application_id] = slotsById[v.application_id] || []).push(v);
  });
  ...
  view(
    ...
    '<nav class="rail" id="tabs" aria-label="Sections">' +
      ...
      '<button class="rnav is-on" data-tab="apps" data-stage="" type="button">' + ICON.apps +
        'Applications<span class="rnav__n" id="tab-apps"></span></button>' +
      /* One entry per stage, nested under Applications above.

         The queue used to be six headings down one scrolling page, which asks
         you to hold in your head which of them you have already passed. A
         stage with nobody in it was drawn as nothing at all, so "nobody is at
         interview" and "I have scrolled past interview" looked identical.

         Every stage is drawn here whether or not anybody is in it, because the
         count beside it IS the answer to that question, and an entry that
         appears and disappears cannot be aimed at. */
      QUEUE_ORDER.map(function (k) {
        return '<button class="rnav rnav--sub" data-tab="apps" data-stage="' + k +
          '" type="button">' + esc(QUEUE_HEAD[k]) +
          '<span class="rnav__n" id="rn-' + k + '"></span></button>';
      }).join("") +
      /* Somewhere for a status this list has never heard of. Filled in by
         paintStageCounts once the rows are known; a row nobody can find is
         worse than a row in the wrong place. */
      '<span id="rn-extra"></span>' +
  ...
  document.getElementById("rows").addEventListener("click", function (e) {
    ...
    var ivo = e.target.closest("[data-ivoffer]");
    if (ivo && !ivo.disabled) { ivOffer(ivo); return; }
    var ivc = e.target.closest("[data-ivconfirm]");
    if (ivc && !ivc.disabled) {
      /* The link box beside this button, if it was filled in. Read from the
         same span so two confirmed rows on one screen cannot hand each other
         the wrong URL. */
    ...
    var ivl = e.target.closest("[data-ivlink]");
    if (ivl && !ivl.disabled) {
      ...
      ivAct(ivl, "rpc/set_application_interview_link",
        { slot: ivl.getAttribute("data-ivlink"), url: lval }, "Sending…");
      return;
    }
    var ivd = e.target.closest("[data-ivdrop]");
    var ivm = e.target.closest("[data-ivresched]");
    if (ivm && !ivm.disabled) { ivResched(ivm); return; }
    var ivx = e.target.closest("[data-ivcancel]");
    if (ivx && !ivx.disabled) { ivCancel(ivx); return; }
    if (ivd && !ivd.disabled) { ivAct(ivd, "rpc/withdraw_application_slot",
      { slot: ivd.getAttribute("data-ivdrop") }, "Withdrawing…"); return; }
  ...
  /* Open on the first stage with somebody in it rather than on the overview:
     the queue is a list of people to deal with, and a page of totals is not
     the thing you came to do. Nobody applied at all lands on the overview,
     which then says so with its own zeroes. */
  STAGE = firstBusyStage();
  var land = document.querySelector('[data-stage="' + STAGE + '"]');
  if (land) {
    document.querySelectorAll("#tabs [data-tab]").forEach(function (x) {
      x.classList.toggle("is-on", x === land);
    });
    var h = document.querySelector(".adm__top h2");
    if (h) h.textContent = QUEUE_HEAD[STAGE] || "Applications";
  }
  paint();
}
