...
function loadClient(email, rows) {
  return Promise.all([
    /* application_public, not applications. 018 grants the whole applications
       table to authenticated, so a policy letting a client read that row would
       hand them the email, the phone number, the CV and every skill rating
       with it. 041 mirrors the one field they are entitled to into its own
       table and leaves the rest where it is. */
    api("placements?select=id,application_id,status,started_on,ended_on,hours_per_week," +
        "trial_weeks,application_public(name)&order=started_on.desc.nullslast"),
...
function clientBlock() {
  ...
  /* Normalised the same way /admin normalises client_private: the embed is
     one-to-one through a primary key, so PostgREST nests an object, but which
     shape you get is a property of the constraint rather than of this code. */
  var ap = live.application_public;
  if (Array.isArray(ap)) ap = ap[0];
  var who = (ap && ap.name) || "your assistant";
...
function ensureSheet() {
  ...
  return api("timesheets", {
    method: "POST",
    headers: { Prefer: "return=representation" },
    body: { application_id: APP.id, week_starts_on: VIEW }
  }).then(function (rows) {
    var s = (rows || [])[0];
