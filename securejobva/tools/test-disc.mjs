...
check("sql/021 and careers.html are current with the item bank", () => {
  ...
  const onDisk = readFileSync("sql/021-disc.sql", "utf8");
  assert(onDisk === sqlText(), "sql/021-disc.sql is stale — run node tools/build-disc.mjs");
...
check("the scoring map in the SQL is the item bank", () => {
  const sql = readFileSync("sql/021-disc.sql", "utf8");
  const block = sql.match(/map\s+jsonb\s*:=\s*'\[([\s\S]*?)\]'::jsonb/);
  assert(block, "no scoring map found in sql/021-disc.sql");
